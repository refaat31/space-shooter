...
#os.environ['SDL_VIDEO_CENTERED'] = "%d"%x
import pygame
import time
import random
...
def main():
    ...
    while run:
        ...
        if (keys[pygame.K_a] or keys[pygame.K_LEFT]) and player.x>0:
            player.x -= player_vel
        if (keys[pygame.K_d] or keys[pygame.K_RIGHT]) and player.x<(WIDTH-player.get_width()):
            player.x += player_vel
        if (keys[pygame.K_w] or keys[pygame.K_UP]) and player.y>0:
            player.y -= player_vel
        if (keys[pygame.K_s] or keys[pygame.K_DOWN]) and player.y<(HEIGHT-player.get_height()+10):
            player.y += player_vel  
        if keys[pygame.K_SPACE]:
            player.shoot()


        #making a copy of the enemies list before looping 
        #through it
        for enemy in enemies[:]:
